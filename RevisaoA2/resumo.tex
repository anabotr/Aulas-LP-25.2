...
• \textbf{df.groupby("coluna")["valor"].sum()} agrupa os valores da coluna "coluna" e soma os 
valores de "valor" de cada grupo. É possível aplicar mais de uma função: 
\textbf{df.groupby("categoria")["vendas"].agg(["sum", "mean", "max"])} ou agrupar por índice 
hierárquico: \textbf{df.groupby(["ano", "mes"])["vendas"].sum()} $\rightarrow$ cria níveis
hierárquicos.
